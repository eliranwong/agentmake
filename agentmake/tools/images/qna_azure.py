...
def examine_images_azure(query: str, image_filepath: Union[str, list], **kwargs):
    ...
    if content:
        client = AzureAI.getClient()

        content.insert(0, {"type": "text", "text": query,})

        response = client.chat.completions.create(
            model=os.getenv("AZURE_VISUAL_MODEL") if os.getenv("AZURE_VISUAL_MODEL") else "gpt-4o",
            messages=[
                {
                "role": "user",
                "content": content,
...
TOOL_FUNCTION = examine_images_azure

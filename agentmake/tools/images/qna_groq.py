...
def examine_images_groq(query: str, image_filepath: Union[str, list], **kwargs):
    ...
    if content:
        client = GroqAI.getClient()

        content.insert(0, {"type": "text", "text": query,})

        response = client.chat.completions.create(
            model=os.getenv("GROQ_VISUAL_MODEL") if os.getenv("GROQ_VISUAL_MODEL") else "llama-3.2-90b-vision-preview",
            messages=[
                {
                "role": "user",
                "content": content,
...
TOOL_FUNCTION = examine_images_groq

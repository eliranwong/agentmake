...
TOOL_SCHEMA = {
    "name": "transcribe_youtube_audio_azure",
    "description": "Transcribe YouTube video into text with Azure service",
    "parameters": {
        "type": "object",
        "properties": {
            "url": {
...
def transcribe_youtube_audio_azure(url: str="", **kwargs):
    ...
    if is_youtube_url(url):
        ...
        transcription = agentmake(
            ...
            tool=os.path.join("audio", "transcribe_azure"),
            **kwargs,
        )[-1].get("content", "")
        print("```transcription")
        print(transcription)
...
TOOL_FUNCTION = transcribe_youtube_audio_azure
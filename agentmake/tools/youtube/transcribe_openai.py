...
TOOL_SCHEMA = {
    "name": "transcribe_youtube_audio_openai",
    "description": "Transcribe YouTube video into text with OpenAI service",
    "parameters": {
        "type": "object",
        "properties": {
            "url": {
...
def transcribe_youtube_audio_openai(url: str="", **kwargs):
    ...
    if is_youtube_url(url):
        ...
        transcription = agentmake(
            ...
            tool=os.path.join("audio", "transcribe_openai"),
            **kwargs,
        )[-1].get("content", "")
        print("```transcription")
        print(transcription)
...
TOOL_FUNCTION = transcribe_youtube_audio_openai
...
class AzureDeepseekAI:
    ...
    DEFAULT_API_KEY = os.getenv("AZURE_DEEPSEEK_API_KEY") if os.getenv("AZURE_DEEPSEEK_API_KEY") else "agentmake"
    DEFAULT_API_ENDPOINT = os.getenv("AZURE_DEEPSEEK_API_ENDPOINT") if os.getenv("AZURE_DEEPSEEK_API_ENDPOINT") else "" # e.g. "http://localhost:11434/v1" for Ollama
    DEFAULT_MODEL = os.getenv("AZURE_DEEPSEEK_MODEL")
    DEFAULT_TEMPERATURE = float(os.getenv("AZURE_DEEPSEEK_TEMPERATURE")) if os.getenv("AZURE_DEEPSEEK_TEMPERATURE") else 0.3
    DEFAULT_MAX_TOKENS = int(os.getenv("AZURE_DEEPSEEK_MAX_TOKENS")) if os.getenv("AZURE_DEEPSEEK_MAX_TOKENS") else 4000 # https://docs.github.com/en/github-models/prototyping-with-ai-models#rate-limits

    @staticmethod
    def getClient(api_key: Optional[str]=None, api_endpoint: Optional[str]=None):
        config.azure_deepseek_client = OpenAI(api_key=api_key if api_key else AzureDeepseekAI.DEFAULT_API_KEY, base_url=api_endpoint if api_endpoint else AzureDeepseekAI.DEFAULT_API_ENDPOINT)
    ...
    def getChatCompletion(
        ...
        **kwargs,
        ...
        return AzureDeepseekAI.getClient(api_key=api_key, api_endpoint=api_endpoint).chat.completions.create(
